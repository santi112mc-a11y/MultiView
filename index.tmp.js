...
async function addPanel(){
  ...
  async function load(v){
    ...
    try {
      api().createView({id,url:v,rect:{x:r.left,y:r.top,width:r.width,height:r.height}}).then(result=>{
...
document.getElementById('create').onclick=()=>{
  // La creación de la cuadrícula es 100% local y no espera a Electron.
  // Así la interfaz nunca puede quedar bloqueada si IPC tarda o falla.
  try{
    const raw=Number(document.getElementById('count').value);
    const targetCount=Math.max(1,Math.min(30,Number.isFinite(raw)?Math.floor(raw):4));
    mode=document.getElementById('cols').value;
    showStatus(`Creando ${targetCount} panel${targetCount===1?'':'es'}...`);
    setup.style.display='none';
    workspace.style.display='flex';
    count=0;
    panels.clear();
    grid.innerHTML='';
    for(let i=0;i<targetCount;i++) addPanel();
    setGrid();
    // IPC se inicia después y nunca bloquea la creación de la interfaz.
    if(window.multiview && typeof window.multiview.ping==='function'){
      window.multiview.ping().then(r=>{
        if(r?.ok) showStatus('MultiView listo.');
      }).catch(e=>showStatus(`IPC: ${e.message}`,true));
    }
  }catch(e){
    console.error(e);
